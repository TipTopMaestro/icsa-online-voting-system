  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="p-4">
      ...
      <div class="mb-6">
        ...
        <p class="text-gray-600 mt-1">View and manage all registered voters in the system.</p>
      </div>
      ...
      <section class="bg-white rounded-xl shadow-sm p-4 mb-6 border border-slate-200">
        <div class="flex flex-col md:flex-row md:items-center gap-4">
          ...
          <div class="flex gap-2 items-center">
            <select v-model="filterCourse" @change="applyFilters" class="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700">
              <option :value="null">All Courses</option>
              <option value="BSIT">BSIT</option>
              <option value="BSIS">BSIS</option>
            </select>

            <select v-model="filterYear" @change="applyFilters" class="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700">
            ...
            <select v-model="filterVoted" @change="applyFilters" class="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700">
              <option :value="null">All Status</option>
              <option :value="true">Voted</option>
              <option :value="false">Not Voted</option>
            </select>

            <button @click="applyFilters" class="px-4 py-2 bg-slate-800 text-white rounded-lg hover:bg-slate-700 text-sm">
              Search
            </button>

            <button @click="clearFilters" class="px-3 py-2 text-sm text-slate-600 hover:text-slate-900">
              Clear
            </button>
          </div>
        </div>
      ...
      <!-- Empty State -->
      <section v-if="props.voters.data.length === 0" class="bg-white rounded-xl shadow-sm p-12 text-center border border-slate-200">
        <div class="text-slate-400 mb-4">
          <svg class="w-16 h-16 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/>
          </svg>
        </div>
        <h3 class="text-lg font-semibold text-slate-900 mb-2">No voters found</h3>
        <p class="text-slate-600">No voters match your search criteria or no voters have registered yet.</p>
      </section>
        ...
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-slate-100">
            ...
            <tbody class="divide-y divide-slate-100 text-sm text-slate-700">
              <tr v-for="voter in props.voters.data" :key="voter.id" class="hover:bg-slate-50 transition">
                <td class="py-3 px-4 font-medium text-slate-800">{{ voter.student_id }}</td>
                ...
                <td class="py-3 px-4">
                  <div class="flex items-center gap-3">
                    ...
                    <div class="min-w-0">
                      <div class="font-medium text-slate-800 truncate max-w-xs">{{ voter.user.name }}</div>
                      <div class="text-xs text-slate-500">{{ voter.user.role }}</div>
                    </div>
                  </div>
                </td>
